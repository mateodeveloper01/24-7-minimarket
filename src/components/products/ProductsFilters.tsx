    ...
    <>
      <div className="md:hidden flex flex-col gap-4">
        ...
        <div className="flex gap-4">
          <Popover
            open={open.tipos}
            onOpenChange={() => setOpen({ tipos: !open.tipos, brands: false })}
          >
            <PopoverTrigger asChild>
              <Button
                variant="outline"
                role="combobox"
                aria-expanded={open.tipos}
                      ...
                      tipos.map(({ tipo }: { tipo: string }) => (
                        <CommandItem
                          ...
                          onSelect={(currentValue) => {
                            setTipo(
                              currentValue === tipoState ? "" : currentValue,
                            );
                        ...
                        >
                          <Check
                            className={cn(
                              "mr-2 h-4 w-4",
                              tipoState === tipo ? "opacity-100" : "opacity-0",
                            )}
                          />
                          {tipo}
                        </CommandItem>
          ...
          <Popover
            open={open.brands}
            onOpenChange={() => setOpen({ tipos: false, brands: !open.brands })}
          >
            <PopoverTrigger asChild>
              <Button
                variant="outline"
                    ...
                    {tipos &&
                      tipos.map(({ tipo }: { tipo: string }) => (
                        <CommandItem
                          ...
                          onSelect={(currentValue) => {
                            setBrand(
                              currentValue === brandState ? "" : currentValue,
                            );
                            setOpen({ tipos: false, brands: false });
                          }}
                          className="capitalize"
                        >
                          <Check
                            className={cn(
                              "mr-2 h-4 w-4",
                              brandState === tipo ? "opacity-100" : "opacity-0",
                            )}
                          />
                          {tipo}
                        </CommandItem>
            ...
            {tipos &&
              tipos.map(({ tipo }: { tipo: string }) => (
                <div key={tipo} className="flex items-center gap-3">
                  <Checkbox
                    checked={tipoState === tipo}
                    onCheckedChange={() =>
                      setTipo(tipoState === tipo ? undefined : tipo)
                    }
                  />
              ...
              brands.map(({ brand }: { brand: string }) => (
                <div key={brand} className="flex items-center gap-3">
                  <Checkbox
                    ...
                    onCheckedChange={() =>
                      setBrand(brandState === brand ? undefined : brand)
                    }
                  />
