...
export const getProducts = async ({
	...
	try {
		...
		const data = await prisma.products.findMany({
			...
			skip: offset,
			where: {
				AND: [stock !== undefined ? { stock } : {}, category !== undefined ? { category } : {}, brand !== undefined ? { brand } : {}, tipo !== undefined ? { tipo } : {}]
			},
			orderBy: {
				tipo: 'asc'
			}
...
export const searchProduct = async (filter: string) => {
	...
	const filterConditions: Prisma.productsWhereInput[] = filterParts.map((part) => ({
		OR: [
